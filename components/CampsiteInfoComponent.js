...
function RenderCampsite(props) {   
    ...
    const recognizeComment = ({dx}) => (dx > 70) ? true : false;

    const panResponder = PanResponder.create({
        onStartShouldSetPanResponder: () => true,
        onPanResponderGrant: () => {
            view.current.rubberBand(2000).then(endState => console.log(endState.finished ? 'finished':'canceled'));
        },
        onPanResponderEnd: (e, gestureState) =>{
            ...
            if(recognizeDrag(gestureState)){
                Alert.alert(
                    'Add Favorite',
                    'Are you sure you wish to add' + campsite.name + ' to favorities?',
                    [
                        {
                            text: 'Cancel',
                            style: 'cancel',
